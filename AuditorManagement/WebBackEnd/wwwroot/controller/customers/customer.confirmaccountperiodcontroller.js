angular.module('FOCUSAPP').controller('CustomerConfirmAccountPeriodController', function ($stateParams, $rootScope, $state, $scope, $http, $timeout, $q, GlobalVar, globalService,
    ...
    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceCustomer.confirmCustomerAccountPeriods($scope.TableAdd.PeriodId)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
    ...
    $scope.OnClickConfirmPeriod = function (values) {
        $state.go('customer-confirmaccountreport', { ref_id: values.PeriodId });
    }
