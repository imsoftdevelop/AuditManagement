angular.module('FOCUSAPP').controller('AdminFsgroupcontroller', function ($scope, $http, $timeout, GlobalVar, $q, globalService, serviceMaster) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceMaster.getFSGroupAdmin(), gConfig]).then(function (data) {
                try {
    ...
    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceMaster.deleteFSGroupAdmin($scope.TableAdd.FsgroupId)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceMaster.getFSGroupAdmin()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
    ...
    $scope.OnClickSave = function (after) {
        try {
            if ($scope.TableAdd.Code == undefined || $scope.TableAdd.Code == '')
                ...
                console.log(data);
                var qq = $q.all([serviceMaster.postFSGroupAdmin(data)]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            if (data[0].data.responsecode == 200) {
                                var qq = $q.all([serviceMaster.getFSGroupAdmin()]).then(function (data) {
