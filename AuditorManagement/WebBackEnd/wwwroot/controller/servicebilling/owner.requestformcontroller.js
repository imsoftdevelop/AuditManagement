angular.module('FOCUSAPP').controller('OwnerRequestformController', function ($scope, $http, $timeout, GlobalVar, $q, globalService, serviceBilling, serviceParameter) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceBilling.getRequestFormAllOwner(), gConfig, serviceParameter.getParameterRequestform()]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        ...
                        if (data[2].data.responsecode == '200' && data[2].data.responsedata != undefined && data[2].data.responsedata != "") {
                            $scope.Parameter.TitleRequest = data[2].data.responsedata;
                        }
    ...
    $scope.OnBinding = function () {
        ...
        _.each($scope.TableMain, function (item) { item.RequestDate = formatDateFull(item.RequestDate); })
    ...
    $scope.OnClickAdd = function () {
        $scope.TableAdd = {};
        $scope.TableAdd.SelectTitle = undefined;
        $('#ModalAdd').modal('show');
    }
    ...
    $scope.OnClickTable = function (val) {
        if (val != undefined) {
            $scope.TableAdd = [];
            $scope.TableAdd = val;
              var type = $scope.Parameter.TitleRequest.filter(function (item) { return item.Code == $scope.TableAdd.Subject; });
        if (type.length > 0)
            $scope.TableAdd.SelectTitle = type[0];
                ...
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceBilling.getRequestFormAllOwner()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
    ...
    $scope.OnClickSave = function (after) {
        try {
            if ($scope.TableAdd.SelectTitle == undefined)
                throw "กรุณาเลือกเรื่อง";
            else if ($scope.TableAdd.Detail == undefined || $scope.TableAdd.Detail == '')
                throw "กรุณาระบุรายละเอียด";
            else if ($scope.TableAdd.ContactName == undefined || $scope.TableAdd.ContactName == '')
                throw "กรุณาระบุชื่อผู้ติดต่อ";
            else if ($scope.TableAdd.ContactMobilePhone == undefined || $scope.TableAdd.ContactMobilePhone == '')
                throw "กรุณาระบุเบอร์โทรศัพท์";
            else if ($scope.TableAdd.ContactEmail == undefined || $scope.TableAdd.ContactEmail == '')
                ...
                var data = {
                    RequestKey: $scope.TableAdd.RequestKey,
                    Subject: $scope.TableAdd.SelectTitle.Code,
                    Detail: $scope.TableAdd.Detail,
                    ContactName: $scope.TableAdd.ContactName,
                    ContactMobilePhone: $scope.TableAdd.ContactMobilePhone,
                    ContactEmail: $scope.TableAdd.ContactEmail,
                };
                console.log(data);
                var qq = $q.all([serviceBilling.postNewRequestForm(data)]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            if (data[0].data.responsecode == 200) {
                                var qq = $q.all([serviceBilling.getRequestFormAllOwner()]).then(function (data) {
