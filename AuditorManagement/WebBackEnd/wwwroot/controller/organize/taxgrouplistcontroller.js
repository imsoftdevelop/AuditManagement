angular.module('FOCUSAPP').controller('TaxgrouplistController', function ($scope, $http, $timeout, GlobalVar, $q, globalService, serviceParameter, serviceOrganize) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceOrganize.getMasterTaxgroup(), gConfig]).then(function (data) {
                try {
    ...
    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceOrganize.deleteMasterTaxgroup($scope.TableAdd.TaxgroupId)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceOrganize.getMasterTaxgroup()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
    ...
    $scope.OnClickSave = function (after) {
        try {
            if ($scope.TableAdd.Code == undefined || $scope.TableAdd.Code == '')
                ...
                var data = {
                    TaxgroupId: $scope.TableAdd.TaxgroupId,
                ...
                console.log(data);
                var qq = $q.all([serviceOrganize.postMasterTaxgroup(data)]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            if (data[0].data.responsecode == 200) {
                                var qq = $q.all([serviceOrganize.getMasterTaxgroup()]).then(function (data) {
