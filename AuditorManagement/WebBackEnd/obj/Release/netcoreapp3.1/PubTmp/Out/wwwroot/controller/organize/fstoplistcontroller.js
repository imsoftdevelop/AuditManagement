angular.module('FOCUSAPP').controller('FsToplistController', function ($scope, $http, $timeout, GlobalVar, $q, globalService, serviceParameter, serviceOrganize) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceOrganize.getFSTop(), gConfig]).then(function (data) {
                try {
    ...
    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceOrganize.deleteFSTop($scope.TableAdd.FstopId)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceOrganize.getFSTop()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
    ...
    $scope.OnClickSave = function (after) {
        try {
            if ($scope.TableAdd.Code == undefined || $scope.TableAdd.Code == '')
                ...
                var data = {
                    FstopId: $scope.TableAdd.FstopId,
                ...
                console.log(data);
                var qq = $q.all([serviceOrganize.postFSTop(data)]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            if (data[0].data.responsecode == 200) {
                                var qq = $q.all([serviceOrganize.getFSTop()]).then(function (data) {
