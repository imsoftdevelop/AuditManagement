angular.module('FOCUSAPP').controller('CustomerDocumentListController', function ($stateParams, $rootScope, $state, $scope, $http, $timeout, $q, GlobalVar, globalService,
    serviceCustomer, serviceParameter) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceCustomer.getDocumentList(), serviceCustomer.getCustomerWithKey($scope.UserProfiles.CustomerIdActive)
                , globalService.getConfigSystem()]).then(function (data) {
                    try {
                        ...
                        if (data[1] != undefined && data[1] != "") {
                            if (data[1].data.responsecode == '200' && data[1].data.responsedata != undefined && data[1].data.responsedata != "") {
                                $scope.Customer = data[1].data.responsedata;

                                var qq = $q.all([serviceParameter.getParameterDocumentStyle(), serviceCustomer.getParameterDocumentTypeOwner($scope.Customer.OwnerId)]).then(function (data) {
                                    try {
                                        if (data[0] != undefined && data[0] != "") {
                                            if (data[0].data.responsecode == '200' && data[0].data.responsedata != undefined && data[0].data.responsedata != "") {
    ...
    $scope.initAdd = function () {
        ...
        $("#loading").fadeIn();
        var qq = $q.all([serviceCustomer.getCustomerWithKey($scope.UserProfiles.CustomerIdActive), globalService.getConfigSystem()]).then(function (data) {
            try {
                if (data[0] != undefined && data[0] != "") {
                    if (data[0].data.responsecode == '200' && data[0].data.responsedata != undefined && data[0].data.responsedata != "") {
                        $scope.Customer = data[0].data.responsedata;
                        ...
                        var qq = $q.all([serviceParameter.getParameterDocumentStyle(), serviceCustomer.getParameterDocumentTypeOwner($scope.Customer.OwnerId)]).then(function (data) {
                            try {
                                ...
                                if ($stateParams.ref_id != undefined && $stateParams.ref_id != '') {
                                    ...
                                    var qq = $q.all([serviceCustomer.getDocumentListWithKey($stateParams.ref_id)]).then(function (data1) {
                                        try {
    ...
    $scope.OnClickTable = function (val) {
        if (val.CreatedBy == $scope.UserProfiles.UserId)
            $state.go('customer-documentadd', { ref_id: val.DocumentListId });

    }

    $scope.OnClickAdd = function () {
        $state.go('customer-documentadd');
    }
    ...
    $scope.OnClickView = function (val) {
        if (val.DocumentStyleId == 'DS002')
            ...
            window.open($scope.ConfigSystem.ImagePath + $scope.Customer.OwnerId + $scope.ConfigSystem.FilePath + val.PathFile, "_blank");
    }

    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceCustomer.deleteDocumentList($scope.TableAdd.DocumentListId)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceCustomer.getDocumentList()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
        ...
        try {
            ...
            if ($scope.TableAdd.SelectDocumentType == undefined)
                ...
                var data = {
                    ...
                    BranchId: $scope.Customer.BranchId,
                    OwnerId: $scope.Customer.OwnerId,
                            ...
                            if (data[0].data.responsecode == 200) {
                                ...
                                if ($scope.SelectedFiles != undefined && $scope.Upload == 1) {
                                    try {
                                        ...
                                        $.ajax(
                                            {
                                                url: baseURL + "Customers/UploadFileDocument?DocumentId=" + data[0].data.responsedata.DocumentListId + "&OwnerId=" + $scope.Customer.OwnerId,
                                                data: formData,
                                                processData: false,
                                                contentType: false,
                                                type: "POST",
                                                success: function (data) {
                                                    if (after == 'back') {
                                                        $state.go('customer-documentlist');
                                                    }
                                                    else {
                                                        $scope.OnInitAddNew();
                                                        $scope.$apply();
