angular.module('FOCUSAPP').controller('AdminAuditprogramcontroller', function ($scope, $http, $timeout, GlobalVar, $q, globalService, serviceMaster) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceMaster.getAuditProgramAdmin(), gConfig, serviceMaster.getAuditProgramFSGroupAdmin()
                , serviceMaster.getFSGroupAdmin()]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            ...
                            if (data[3].data.responsecode == '200' && data[3].data.responsedata != undefined && data[3].data.responsedata != "") {
                                ...
                                globalService.SetupDropdownDefault($scope.Parameter.FSGroup);
                            }
    ...
    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceMaster.deleteAuditProgramAdmin($scope.TableAdd.Auditprogramid)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceMaster.getAuditProgramAdmin(), serviceMaster.getAuditProgramFSGroupAdmin()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
    ...
    $scope.OnClickSave = function (after) {
        try {
            if ($scope.TableAdd.Code == undefined || $scope.TableAdd.Code == '')
                ...
                console.log(data);
                var qq = $q.all([serviceMaster.postAuditProgramAdmin(data)]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            if (data[0].data.responsecode == 200) {
                                var qq = $q.all([serviceMaster.getAuditProgramAdmin(), serviceMaster.getAuditProgramFSGroupAdmin()]).then(function (data) {
    ...
    $scope.OnClickTableRelation = function (values) {
        if (values != undefined) {
            ...
            if ($scope.TableAddRelation.FsgroupCode != undefined) {
                ...
                var type = $scope.Parameter.FSGroup.filter(function (item) { return item.Code == ''; });
                if (type.length > 0)
                    $scope.TableAddRelation.SelectFSGroup = type[0];
            }
    ...
    $scope.OnClickSaveRelation = function () {
        try {
            var data = {
                FsgroupId: $scope.TableAddRelation.SelectFSGroup.Code == '' ? undefined : $scope.TableAddRelation.SelectFSGroup.FsgroupId,
                Auditprogramid: $scope.TableAddRelation.AuditProgramId,
            };

            console.log(data);
            var qq = $q.all([serviceMaster.postAuditProgramFSGroupAdmin(data)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceMaster.getAuditProgramFSGroupAdmin()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
