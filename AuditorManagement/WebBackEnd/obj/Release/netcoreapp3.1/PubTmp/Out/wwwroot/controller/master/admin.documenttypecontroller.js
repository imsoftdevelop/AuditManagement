angular.module('FOCUSAPP').controller('AdminDocumenttypecontroller', function ($scope, $http, $timeout, GlobalVar, $q, globalService, serviceMaster) {
    ...
    $scope.init = function () {
        try {
            ...
            $("#loading").fadeIn();
            var qq = $q.all([serviceMaster.getDocumentTypeAdmin(), gConfig]).then(function (data) {
                try {
    ...
    $scope.OnClickConfirm = function () {
        try {
            var qq = $q.all([serviceMaster.deleteDocumentTypeAdmin($scope.TableAdd.Documentid)]).then(function (data) {
                try {
                    if (data[0] != undefined && data[0] != "") {
                        if (data[0].data.responsecode == 200) {
                            var qq = $q.all([serviceMaster.getDocumentTypeAdmin()]).then(function (data) {
                                try {
                                    if (data[0] != undefined && data[0] != "") {
                                        if (data[0].data.responsecode == 200) {
                                            $scope.TableMain = data[0].data.responsedata;
    ...
    $scope.OnClickSave = function (after) {
        try {
            if ($scope.TableAdd.Code == undefined || $scope.TableAdd.Code == '')
                ...
                var data = {
                    Documentid: $scope.TableAdd.Documentid,
                ...
                console.log(data);
                var qq = $q.all([serviceMaster.postDocumentTypeAdmin(data)]).then(function (data) {
                    try {
                        if (data[0] != undefined && data[0] != "") {
                            if (data[0].data.responsecode == 200) {
                                var qq = $q.all([serviceMaster.getDocumentTypeAdmin()]).then(function (data) {
